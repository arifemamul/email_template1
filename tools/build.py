...
# A second document, built the same way. It reads the reports the game keeps and is linked from
# nowhere - see src/js/90-reports-page.js for what that does and does not protect.
REPORTS_SRC = SRC / 'reports.html'
...
# A folder of its own, so the address is a plain `…/reports/` rather than a filename, and so
# anything this page ever needs beside it has somewhere to live without landing next to the game.
REPORTS_OUT = REPO / 'docs/reports/index.html'
...
DOCUMENTS = (SOURCE, REPORTS_SRC)


def parts(document=SOURCE):
    """Every authored part, in the order the document lists them."""
    return [m.group(1) for m in INCLUDE.finditer(document.read_text(encoding='utf-8'))]


def assemble(document=SOURCE):
    """
    ...
    """
    missing = [p for p in parts(document) if not (SRC / p).exists()]
    if missing:
        raise SystemExit(f'{document.name} includes files that do not exist: ' + ', '.join(missing))
    return INCLUDE.sub(lambda m: (SRC / m.group(1)).read_text(encoding='utf-8').rstrip('\n'),
                       document.read_text(encoding='utf-8'))


def orphans():
    """Authored parts no document includes - so a new file cannot go unwired."""
    listed = {p for d in DOCUMENTS for p in parts(d)}
...
def write_builds(levels):
    ...
    # The reports page. Built from source the same way so it cannot drift from the game's
    # palette, and deliberately not added to the service worker's shell: it is a page someone
    # opens on purpose, not part of the app that installs.
    before_reports = REPORTS_OUT.read_text(encoding='utf-8') if REPORTS_OUT.exists() else ''
    reports_built = minify(assemble(REPORTS_SRC))
    REPORTS_OUT.parent.mkdir(parents=True, exist_ok=True)
    REPORTS_OUT.write_text(reports_built, encoding='utf-8')

    return (False, before_web != built, before_live != live_src,
            before_reports != reports_built)
...
def main(argv):
    ...
    if command == 'check':
        ...
        stray = orphans()
        if stray:
            print('\nauthored parts no document includes:\n  ' + '\n  '.join(stray))
            return 1
    ...
    if command == 'stale':
        ...
        # Both documents, or the guard only covers the one the tests happen to open.
        want_reports = minify(assemble(REPORTS_SRC))
        have_reports = REPORTS_OUT.read_text(encoding='utf-8') if REPORTS_OUT.exists() else ''
        if stale_levels or want != have or want_reports != have_reports:
            print('STALE: docs/ is not what src/ would build right now')
            if stale_levels:
                print('  the level table the catalogue produces is not the one in src/js')
            if want != have:
                print(f'  index.html differs: {len(want)} chars from source, {len(have)} on disk')
            if want_reports != have_reports:
                print(f'  {REPORTS_OUT.relative_to(REPO)} differs: {len(want_reports)} chars '
                      f'from source, {len(have_reports)} on disk')
    ...
    if command == 'build':
        if failures:
            ...
            return 1
        kt_changed, web_changed, live_changed, reports_changed = write_builds(levels)
        ...
        print(f'  {REPORTS_OUT.relative_to(REPO)}  {"updated" if reports_changed else "unchanged"}')
