...
def voice(levels):
    ...
    words, letters, first = [], [], {}
    for i, level in enumerate(levels, 1):
        for symbol in [s for _, s in level['teaches']]:
            # A vowel sign cannot be said alone, so record the tile that carries it - the same
            # substitution the page makes when it draws the "new letter" chip.
            carrier = next((a for a in level['tiles'] if symbol in a and a != symbol), symbol)
            if carrier not in letters and carrier not in first:
                letters.append(carrier)
                first[carrier] = i
        for word in level['words'] + level['extras']:
    ...
    print(f'// {len(letters)} letters and {len(words)} words, in the order a learner meets them.')
    print('const VOICE_CLIPS = {')
    for group, items in (('letters and signs', letters), ('words', words)):
        print(f'  // -- {group} --')
        for item in items:
            theme = theme_of(item)
            note = f'  // level {first[item]}' + (f', {theme}' if theme else '')
            print(f'  "{item}": "audio/{item}.opus",{note}')
    print('};')
    print(f'\n// {len(letters) + len(words)} clips. Roughly '
          f'{(len(letters) + len(words)) * 4 // 1000 + 1} MB as Opus at 24 kbps, so they fit '
          f'inside the single-file build as data: URIs.')
