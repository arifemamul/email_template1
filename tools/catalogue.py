...
# And the guide's own words. The app showed 614 Bengali words it would not let a child spell -
# the শব্দ গড়া sums, the words listed under each কার, one for every ফলা and যুক্তবর্ণ form, and
# the বারোখড়ি's fillers. Generated by tools/guide_levels.py, which places every one that
# satisfies `validate` and leaves the rest in the guide, where they were doing their own job.
_GUIDE = json.loads((HERE / 'levels-guide.json').read_text(encoding='utf-8'))
# And a vocabulary list handed in from outside: at least four words per letter of the alphabet,
# the words a primary-school child is taught, each with its English meaning. Half of it was
# already on a board; tools/vocab_levels.py sets what was not, and picks up the drawable guide
# words the file above could not place. Reads tools/word-list-school.json, which holds the list
# exactly as it arrived.
_VOCAB = json.loads((HERE / 'levels-vocab.json').read_text(encoding='utf-8'))

_AUTHORED = (_DATA['levels'] + _EXTRA['levels'] + _POOL['levels'] + _VOWELS['levels']
             + _OI['levels'] + _GUIDE['levels'] + _VOCAB['levels'])
