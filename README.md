...
- **Web build - this is the live one.** One self-contained file:
  ...
  [`docs/live.html`](docs/live.html) is the same page with the `<!doctype>`/`<head>`/`<body>`
  wrapper stripped, for hosts that supply their own. It is generated by `build.py build` from
  `index.html` - never edit it directly.
